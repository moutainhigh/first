...
public class DppmReportDao extends iBatis3DaoImpl implements IDppmReportDao {
	...
	public String queryKpi(String date) {
		...
		for (int type = 1 ; type <= 2 ; type++){
			...
			if(date.length() == 10) {
				...
				//最近一周访问人数
				map.put("chartList", kpiChartInWeek(DateUtil.queryBeforeday(date,6),date,type));
				...
				//当前月
				if(month.equals(cudate.substring(0,7))) {
					//当前日期
					String cdate = DateUtil.queryBeforeday(cudate, 1);
					day = Integer.parseInt(cdate.substring(8,10));
				} else {
					//其他月 day等于月底30号
	...
	public List<Integer[]> visitKpiMonth(String date,int type,int flag) {
		...
		if(date.length() == 10) {
			...
			//本月
			if(cdate.substring(0,7).equals(month)) {
				//当天 为天数
				num = Integer.parseInt(cdate.substring(8,10));
			} else {
				// 月的天数
				num = Integer.parseInt(DateUtil.queryLastdayOfMonth(month).substring(8,10));
			}
		} else {
			this.logger.info("按月统计：---" + date);
			month = date.substring(0,4);
			//本年的
			if(cdate.substring(0,4).equals(month)) {
				//当月
				num = Integer.parseInt(cdate.substring(5,7));
			} else {
				num = 12;
			}
		}
...
//		sql += " group by emp_code ) a ";
	...
	public List<Integer[]> querySqlInt(String sql) {
		...
		list.add(new Integer[]{11,0});
		list.add(new Integer[]{15,0});
		list.add(new Integer[]{17,0});
		//执行查询
		this.template.query(sql,new RowMapper<String[]>() {
			@Override
			public String[] mapRow(ResultSet rs, int arg1)
