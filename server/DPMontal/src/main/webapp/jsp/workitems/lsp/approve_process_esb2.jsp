...
<%@page import="com.deppon.lsp.module.mobilesecondworkflowlist.controlcenter.shared.domain.AttachEntity"%>
<%@page import="com.deppon.lsp.module.mobilesecondworkflowlist.controlcenter.server.service.ApprovalInfo"%>
<%@page import="java.util.List"%>
<%@ page language="java"  pageEncoding="UTF-8"%>

<% 
	WorkflowSecondEntity infoForApprovalInfoCommon = (WorkflowSecondEntity)request.getAttribute("lspResponseEntity");
	
	List<ApprovalInfo> ApprovalInfoList = infoForApprovalInfoCommon.getApprovalInfoList();
		...
		<table  width="100%">
			...
			<tr class="files">
				<th colspan="2"><a class="yellow" id="showFile">查看附件列表</a></th>
					...
					<%	
						List<AttachEntity> fileArray = infoForApprovalInfoCommon.getAttachList();
						int length = fileArray == null ? 0:fileArray.size();
						for(int i = 0 ; i < length  ; i++) {
							%>
					<tr class='toFile' style="display: none">
						<td>
							<br><a href = "<%=fileArray.get(i).getFileURL()%>" style="color:red"><%=fileArray.get(i).getFileName()%></a>&nbsp;
						</td>
					</tr>
					<%} %>
   			 <tr class="toFile" style="display: none">
