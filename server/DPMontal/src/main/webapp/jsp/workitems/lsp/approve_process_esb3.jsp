...
<!-- 附件 -->
		<table  width="100%">
			...
			<tr class="files">
				<th colspan="2"><a class="yellow" id="showFile">查看附件列表</a></th>
					...
					<tr class='toFile' style="display: none">
						<td>
							<br><a href = "<%=attachList.get(i).getFileURL()%>" style="color:red"><%=attachList.get(i).getFileName()%></a>&nbsp;
						</td>
					</tr>
					<%} %>
   			 <tr class="toFile" style="display: none">
			...
			%>
				 <tr <%=i==0?"class='sh'":"class='topLine sh'" %>><th>审批时间:</th> <td><%=appInfo.getApprovedate()%></td> </tr>
