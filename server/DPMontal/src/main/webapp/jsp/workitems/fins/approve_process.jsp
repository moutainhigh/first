...
%>
		...
		<tbody class="dpFiles">
			...
			<tr class="files">
				<th colspan="2"><a class="yellow" id="showFile">查看附件列表</a></th>
					...
					<tr class='toFile' style="display: none">
						<td>
							<br><a href = "downloadFiles?syscode=<%=request.getAttribute("syscode")%>&filePath=<%=fileInfo.get(i).getSavePath()%>&fileName=<%=fileInfo.get(i).getFileName()%>" style="color: red"><%=fileInfo.get(i).getFileName()%></a>&nbsp;
						</td>
					</tr>
					<%} %>
   			 <tr class="toFile" style="display: none">
			...
			%>
				 ...
				 <tr class="sh"><th>审批人岗位</th> <td><%=appInfo.getApproverPosition() %></td></tr>
				 <tr class="sh"><th>审批结果:</th> <td><%=appInfo.getIsagree()%></td> </tr>
