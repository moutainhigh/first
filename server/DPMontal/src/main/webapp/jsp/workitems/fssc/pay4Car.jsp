...
<div id="list">
	...
	<div id="div2">
    	...
    	<div class="tableBox">
			...
			<tr><th>工作流:</th><td>FOSS报账单-专线外请车异常扣款支付</td></tr>
			...
			<tr <%=i==0 ? "" : "class='topLine'" %>><th>费用承担部门:</th><td><%= mobileLineList[i].getCostCenterName()%></td></tr>
			<tr><th>费用类型:</th><td><%= mobileLineList[i].getScName()%></td></tr>
			<tr><th>业务发生日期:</th><td><%=FormatUtil.formatDate(mobileLineList[i].getBizOccurDate(), "yyyy-MM-dd") %></td></tr>
			<tr><th>报账金额:</th><td><%= FormatUtil.formatMoney(mobileLineList[i].getActualAmountStd())%></td></tr>
			<tr class="changeByAccount"><th>核定金额:</th><td><%= FormatUtil.formatMoney(mobileLineList[i].getRetifyAmount())%></td></tr>
			<tr><th>费用说明:</th><td><%= mobileLineList[i].getCostExplain() == null ? "" : mobileLineList[i].getCostExplain()%></td></tr>
				   <%} %>
		  </table>
