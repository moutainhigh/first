	...
	<div id="div2">
    	...
    	<div class="tableBox">
	    	<table width="100%">
				...
				<tr><th>工作流:</th><td>FOSS专线外请车-付款</td></tr>
				...
				<tr><th>申请时间:</th><td><%=base.getApplyDateStr()%></td></tr>
				...
				<tr><th>最迟汇款日期:</th><td><%="".equals(base.getLatestRemitDateStr())?FormatUtil.formateDateAdd(new Date(), 2):base.getLatestRemitDateStr()%></td></tr>
				...
				<tr><th>报账金额:</th><td><%=FormatUtil.formatMoney(base.getPayAmount())%></td></tr>
				<tr><th>付款金额:</th><td><%=FormatUtil.formatMoney(base.getPayAmount())%></td></tr>
				<tr><th>核定金额:</th><td><%=FormatUtil.formatMoney(base.getPayAmount())%></td></tr>
				<tr><th>币种:</th><td><%=base.getCurrency()%></td></tr>
				...
				<tr <%=i==0 ? "" : "class='topLine'" %>><th>出发部门:</th><td><%= mobileLineList[i].getStartDeptName()%></td></tr>
				<tr><th>到达部门:</th><td><%= mobileLineList[i].getArriveDeptName()%></td></tr>
				<tr><th>费用类型:</th><td><%= mobileLineList[i].getScName()%></td></tr>
				<tr><th>车次号:</th><td><%= mobileLineList[i].getCarNum()%></td></tr>
				<tr><th>车牌号:</th><td><%= mobileLineList[i].getLicensePlateNum()%></td></tr>
				<tr><th>司机:</th><td><%= mobileLineList[i].getDriverName()%></td></tr>
				<tr><th>总额:</th><td><%= mobileLineList[i].getTotalAmount()%></td></tr>
				<tr><th>出发付金额:</th><td><%= mobileLineList[i].getStartPayAmount()%></td></tr>
				<tr><th>到达付金额:</th><td><%= mobileLineList[i].getArrivePayAmount()%></td></tr>
				<tr><th>增减变化:</th><td><%= mobileLineList[i].getRagulationChange() == null?"":mobileLineList[i].getRagulationChange()%></td></tr>
				<tr><th>押回单:</th><td><%= "Y".equals(mobileLineList[i].getIsReturnBill())?"是":"否"%></td></tr>
				<tr  class="changeByAccount"><th>核定金额:</th><td><%= mobileLineList[i].getRetifyAmount()%></td></tr>
				   <%} %>
		  </table>
