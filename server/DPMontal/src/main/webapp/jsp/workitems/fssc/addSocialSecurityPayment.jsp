    	...
    	<div class="tableBox">
	    	<table width="100%">
	    		...
	    		<tr>
				   ...
				   <td id="workid"><%=baseEntity.getClaimNo() %><input type="hidden" id="workid" value="<%=baseEntity.getWfInstanceId() %>"></td>
				</tr>
				<tr><th>工作流:</th><td>薪酬福利报账-社保管理费（派遣）—付款</td></tr>				
				...
				<tr><th>支付方式:</th><td><%=baseEntity.getPayType() %></td></tr>
				<tr><th>币种:</th><td><%=baseEntity.getCurrency() %></td></tr>
				<tr><th>收款方:</th><td><%=baseEntity.getAccountName() %></td></tr>
				<tr><th>最迟汇款日期:</th><td><%=baseEntity.getLatestRemitDateStr()==null?"":baseEntity.getLatestRemitDateStr() %></td></tr>
				<tr><th>手机号码:</th><td><%=baseEntity.getContactInfo() %></td></tr>
				...
				<tr><th>报账金额:</th><td><%=FormatUtil.formatMoney(baseEntity.getApplyAmount()) %></td></tr>
				<tr><th>付款金额:</th><td><%=FormatUtil.formatMoney(baseEntity.getPayAmount()) %></td></tr>
				<tr><th>核销金额:</th><td><%=FormatUtil.formatMoney(baseEntity.getAlreadyWritedAmount()) %></td></tr>
				<tr><th>币种:</th><td><%=baseEntity.getCurrency() %></td></tr>
				<tr><th>付款金额大写:</th><td id="amountCNY"></td></tr>
				<tr><th>账户性质:</th><td><%=baseEntity.getAccountNature() %></td></tr>
				<tr><th>银行账号:</th><td><%=baseEntity.getAccount() %></td></tr>
				<tr><th>开户支行:</th><td><%=baseEntity.getSubbankName() %></td></tr>
				...
				<tr><th>费用期间:</th><td><%=line.getCostPeriod() %></td></tr>
				...
				<tr><th>管理费:</th><td><%=FormatUtil.formatMoney(line.getManageFee())%></td></tr>
				<tr><th>个人社保扣款:</th><td><%=FormatUtil.formatMoney(line.getPersonalTotSocialSecurity())%></td></tr>
				<tr><th>单位社保扣款:</th><td><%=FormatUtil.formatMoney(line.getCompTotSocialSecurity())%></td></tr>
				<tr><th>社保卡费用:</th><td><%=FormatUtil.formatMoney(line.getSocialSecurityCardFee())%></td></tr>
				<%
					}
				%>				
		    </table>
