...
public class TaskLogDao extends iBatis3DaoImpl implements ITaskLogDao {
	 ...
	 * @return 是否保存成功
	 */
	public static final int NUMBER_OF_ONEHUNDRED = 100;
	...
	public int addtasklog(TaskLogEntity e) {
		...
		try {
			...
			int isCheck = 0;
			// 任务完成进度 》100 时 需要审核 
			if (e.getPercentComplete() >= NUMBER_OF_ONEHUNDRED) {
				// 赋值为1 需要审核
				isCheck = 1;
			}
			flag = this.template.update("insert into dotp_tm_task_log(task_log_worked_hour,task_log_desc,task_log_email," +
					...
					+ "," + e.getPercentComplete() + ",now(),0," + isCheck + ",'" + e.getEmpcode() + "') ");
			//日志保存成功 后更新任务完成进度及 任务状态
			if (flag == 1) {
				String sql = "update dotp_tm_tasks set id=id ";
				//任务状态    已暂存:1、已分配:2、进行中:3、已暂停:4、已停止:5、已提交:6、已完成 7
				//默认任务进行中
				int taskStatus = NUMBER_OF_THREE;
				//任务实际功能点 当完成100%时 需更新实际功能点
				int taskRealPoint = 0;
				// 完成进度>=100时  赋值任务状态为6 已提交
				if(e.getPercentComplete() >= NUMBER_OF_ONEHUNDRED) {
					// 已提交
					taskStatus = NUMBER_OF_SIX;
					// 对任务实际功能点赋值
					taskRealPoint = e.getRealPoint();
				} else {//未完成时 更新任务进度 完成不更新
					sql += ",task_percent_complete=" + e.getPercentComplete();
				}
				sql += ",task_status=" + taskStatus + ",task_real_point=" + taskRealPoint 
						+ ",task_is_check=" + isCheck + " where id=" + e.getTaskId();
				logger.info(sql);
				// 执行 更新任务 sql
