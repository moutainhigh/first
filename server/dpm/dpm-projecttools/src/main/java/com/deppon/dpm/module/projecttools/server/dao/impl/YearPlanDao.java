...
public class YearPlanDao extends iBatis3DaoImpl implements IYearPlanDao {
        ...
        // 根据 sql获得　年度规划列表信息
     	this.dppmTemp.query(sql,
     		...
     		public AnnualPlanEntity mapRow(ResultSet rs, int arg1)
     			...
     			//添加战略类项目 3
     			if(rs.getInt("gh_level") == NUMBER_OF_THREE) {
     				zlList.add(info);
     			} else {
     				itList.add(info);
     			}
	...
	public String projInfo(int projectId,int ghType) {
		...
		} else if (ghType == NUMBER_OF_THREE) {
			// 基础设施型
			       ////应用部门
			sql += ",(select Group_concat(dept_name) from dotp_departments where dept_benchmark_code in (" +
		...
		sql += " from dotp_gh_projects s " + extra + " where s.project_id=" + projectId;
		// 根据 sql获得　规划详情
     	...
     	sql = "select r.*,(select label from dotp_names where name='gh_role' and value=r.emp_role) emprole," +
     			...
     			"from dotp_gh_project_res1 r where r.project_id=" + projectId;
     	// 根据 sql获得　人员类
...
//     	}
        //人员类 费用合计 sql
     	sql = "select sum(r.total_cnt) totalcnt from   dotp_gh_project_res1 r where r.project_id=" + projectId;
    	...
    	map.put("res1Total", res1Total);
     	...
     	sql = "select r.*,(select label from dotp_names where name='gh_role' and value=r.cost_for) costfor " +
     			"from dotp_gh_project_res2 r where r.project_id=" + projectId;
     	// 根据 sql获得　非人员类
...
//     	}
     	//人员类 费用合计 sql
        sql = "select sum(r.cost_cnt) totalcnt from   dotp_gh_project_res2 r where r.project_id=" + projectId;
