...
public class TaskLogService implements ITaskLogService {
	...
	public void mailSend(TaskLogEntity log) {
		...
		if(email != null) {
			...
			for(int i = 0 ; i < sm.length ; i++) {
				if(sm[i].split("\\|").length >= NUMBER_OF_THREE) {
			...
			//设置邮件主题
			String title = "[" +task.getTaskProjectName() + "]" + task.getTaskName() + "进度更新为" + log.getPercentComplete();
			//邮件内容
			...
			info.append("已工作小时数：" + log.getWorkedHour() + "<br>计划进度：" + ddd.divide(ddd2,2,BigDecimal.
					ROUND_HALF_UP).doubleValue() * NUMBER_OF_ONEHUNDRED + "%<br>实际进度：" + log.getPercentComplete()+"%<br>");
	...
	public void mailSendTask(TasksEntity task) {
		...
		if(email != null) {
			...
			//设置邮件主题
			String title = "[" + task.getTaskProjectName()+ "]" + task.getTaskName() + "  已添加";
			//邮件内容
			StringBuffer info = new StringBuffer();
			//项目类任务时 添加项目类型等的提醒
