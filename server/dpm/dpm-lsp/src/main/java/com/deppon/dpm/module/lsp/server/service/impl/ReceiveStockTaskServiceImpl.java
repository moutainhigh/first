...
//							 nce.setIsTxtNews(DpmConstants.NO);
						 	...
						 	nce = new NewsCenterEntity("2", NUMBER_OF_THREE, 0, 1,"固定资产");
...
//					 }
				...
				//固定资产更新由每次推送一条变更多条
				 List<PushAssetClearScrapeEntity> scrapeEntityList=JsonUtil.jsonToList(JsonUtil.jsonGetValueBykey(strJson, "requestEntity"), PushAssetClearScrapeEntity.class);
				
				 String docNoStr ="";
				
				logger.info("======资产更新pc端传过来json字符串是=>"+strJson);
				if(scrapeEntityList!=null && scrapeEntityList.size()>0){
					    for(PushAssetClearScrapeEntity pacse :scrapeEntityList){
							...
							if (flag>0) {//保存成功后推送
								...
								if("".equals(docNoStr)){
									docNoStr=pacse.getDocNo();
								}else{
									docNoStr=docNoStr+","+pacse.getDocNo();
								}
								/*String []str=code.split("\\|");
								for (String empCode:str) {//根据审批人集合推送					
									nce = new NewsCenterEntity("2", NUMBER_OF_THREE, 0, 1, "固定资产","你有一个资产更新工作流");
...
*/								
				...
				ResponseParameterEntity<String> responseEntity=new ResponseParameterEntity();
				responseEntity.setResultFlag(true);
				responseEntity.setResponseEntity(docNoStr);
				res=JSONObject.toJSONString(responseEntity,SerializerFeature.WriteMapNullValue);
			 }
