package com.deppon.dpm.module.fssc.server.service.requestProcess;
...
public class SendPriorApplicationRequestProcess implements IProcess {
	
	private static Logger logger = Logger
			.getLogger(SendPriorApplicationRequestProcess.class);
	/**
	 *  dao的注入
	 */
	@Resource
	 ...
	 * num
	 */
	private static final int num = MagicNumber.NUM7;
	
	@Override
	public Object process(Object req) throws ESBBusinessException {
		// SendPriorApplicationRequest sendReq =
		// (SendPriorApplicationRequest)req;
		// PriorApplicationEntity prior = new PriorApplicationEntity();
		// BeanUtils.copyProperties(sendReq, prior); // copy一个对象
		/*
		 * JSONObject json = JSONObject.fromObject(req); //JSONArray json =
		 * JSONArray .fromObject(req); String strJson = json.toString();
		...
		logger.info("ReceivePriorApplicationService>>>>>>>>>进入process,传送过来的req:"
				+ strJson);
		// 初始化一个返回消息
		...
		try {
			...
			logger.info("ReceivePriorApplicationService>>>>>>>>>进入process,传送过来的type:"
					+ type);
			if (type == 0) {// 判断事前申请单是什么状态(0:推送数据1:更新状态)
				...
				if (flag > 0) {
					...
					nce = new NewsCenterEntity(prior.getClaimNo(), num, 0, 1,
							"差旅助手");
					// 推送消息
					tpushNewsService.pushUserNews(prior.getApplyEmpNo(),
							"你有一条事前申请单可预订", "事前申请单任务", nce);
					// esb联调
					/*
					 * String
					 * resString=HttpUtil.httpClient(JsonUtil.beanToJSONObject
					 * (JsonUtil
					 * .jsonToEntity(strJson,PriorApplicationEntity.class )),
					 * synUrl, ESB_CODE_Travel_Assistant);
					 * ResponseParameterEntity<?>
					 * rpe=JsonUtil.jsonToEntity(resString,
					 * ResponseParameterEntity.class); if (rpe.isResultFlag()) {
					 * res="{\"resultFlag\":\"1\",\"failureReason\":\"\"}"; }
					 * else{ res=
					 * "{\"resultFlag\":\"0\",\"failureReason\":\"网络异常，请在PC端处理！\"}"
					 * ;
				...
				for (DetailInfoEntity d : detailList) {
					//抽取方法
					setResultForType0(response, result, prior, d);
				}
				...
				PriorApplicationEntity prior = JsonUtil.jsonToEntity(strJson,
						PriorApplicationEntity.class);
				// 更新表头
				int rel = iaDao.updatePriorApplicationEntity(prior);
				if (rel <= 0) {
					result.setResult(false);
					result.setReason("更新事前申请单信息失败，数据库受影响行数小于等于0");
					response.getProcessResultList().add(result);
					// res="{\"resultFlag\":\"false\",\"failureReason\":\"\"}";
					logger.info("[process]-----更新事前申请单信息失败");
				}else{
					// 得到明细信息
					List<DetailInfoEntity> detailList = prior.getDetailInfoList();
					for (DetailInfoEntity d : detailList) {
						//抽取方法
						setResultForTypeOne(response, result, prior, d);
					}
				}
			} else {
			...
			response.getProcessResultList().add(result);
			// return
			// Response.ok("{\"resultFlag\":\"false\",\"failureReason\":\""+e.getMessage()+"\"}").header("ESB-ResultCode",
			// 1).build();
		}
	...
	private void setResultForTypeOne(SendMdmOrgResponse response,
		...
		if (d.getScNo().equals("104002003")) {// 判断是不是飞机费用类型
			...
			for (FlightOrderInfoEntity flight : flightList) {
				...
				flag = iaDao
						.updateFlightOrderInfoEntityByOrderState(flight);
				if (flag > 0) {
					// res="{\"resultFlag\":\"true\",\"failureReason\":\"\"}";
					result.setResult(true);
			...
			for (HotelOrderInfoEntity hotel : hotelList) {
				...
				// 更新对象
				flag = iaDao
						.updateHotelOrderInfoEntityByOrderState(hotel);
				// 判断是否符合条件
				if (flag > 0) {
					// res="{\"resultFlag\":\"true\",\"failureReason\":\"\"}";
					result.setResult(true);
	...
	private void setResultForType0(SendMdmOrgResponse response,
			SendMdmOrgProcessReult result, PriorApplicationEntity prior,
			DetailInfoEntity d) {
		int flag;
