...
import com.deppon.dpm.module.management.util.ComparatorUtil;
...
public class ApplyStoreAction extends BaseAction {
	...
	public void list() {
		...
		try {
			...
			if(null != roleStr){
				roleList = (List<String>)JSONObject.parseObject(roleStr).get("roleCodes");
			}
			// 构造实体
...
//			list = sequenceList(list);
	...
	public void listLimitTen(){
		Result<Object> result = new Result<Object>();
		
	 loginRedisService.get("DPM_JURISDICTION_KEY");
			
		List<ApplyStore> list = iApplyStoreService.listLimitTen();
		result.setData(list);
		// count
		result.setCount(list.size());
		// errorMessage
			...
			if (StringUtils.isNotEmpty(sort)) {
				...
				for (ApplyStore store : list) {
					...
					if ("0".equals(defaultApp)) {
						...
						if(asList.contains(String.valueOf(appId))&&(StringUtils.isEmpty(deviceType) || checkVersionType(versionType, deviceType) == APPTYPE_3)) {
