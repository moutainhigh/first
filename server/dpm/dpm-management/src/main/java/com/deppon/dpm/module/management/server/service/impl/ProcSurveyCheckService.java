...
public class ProcSurveyCheckService implements IProcSurveyCheckService {
	...
	public Response saveprocSurvey(String json) {
		...
		try {
			...
			if(checkNull(re, checkCode, checkName, checkNo, procStatus,
					checkUserNo, principalUserNo, principalName, createUserNo, result) != null){
				return result;
			}
	...
	private Response checkNull(String re, String checkCode, String checkName,
			String checkNo, String procStatus, String checkUserNo,
			String principalUserNo, String principalName, String createUserNo,Response result) {
		//非空校验
		if ("".equals(checkCode)) {

			re = "{\"isSuccess\":0,\"message\":\"传入的参数projectNumber不对\"}";
			result =  Response.ok(re).header("ESB-ResultCode", "1").build();
		}
