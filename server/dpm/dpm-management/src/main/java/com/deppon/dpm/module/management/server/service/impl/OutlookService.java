...
public class OutlookService {
	...
	public boolean validatePassword(String password, String email) {
		...
		try {
			...
			//new 一个新的credentials
			/*if(password.contains(" ")&&StringUtils.isNotEmpty(str)){
				password=password.replace(" ", "+");
			}*/
			 despassword = DES.decryptDES(password);
			 basepassword = new String(Base64.decodeBase64(despassword));
			//String decryptDES = DES.decryptDES(password);
			ExchangeCredentials credentials = new WebCredentials(email,basepassword);
			//set 数据
			...
			//捕获异常
			logger.error("邮箱绑定失败的邮箱是"+email+"报错异常是:" + e.getMessage());
			logger.error("传过来的密码"+password+"des解密后的密码"+despassword+"bas64解密后的密码"+basepassword);
	...
	public String insertOrUpdateEmail(String userId, String password) {
		...
		// 邮箱密码配置表
		String query = "select * from om_email_config where user_id = '" + userId
				+ "'";
		try {
			// sql执行获取配置表的工号
	 ...
	 * @throws EmailNullPointException 
	 * @throws EmailPasswordNullPointException 
	 * @throws EmailPasswordErrorException 
	 ...
	 * @return
	 */
	protected String attachmentMonitor(String app_name) {
		// 返回值
		Result<Object> result = new Result<Object>();
		//判断是否为null
		...
		String update = "update attachment_monitor set down_load_count = down_load_count + 1, updatetime = now() "
				+ "where app_name = ?";
		//update 数据
		jdbcTemplate.update(update, new Object[] {app_name},
