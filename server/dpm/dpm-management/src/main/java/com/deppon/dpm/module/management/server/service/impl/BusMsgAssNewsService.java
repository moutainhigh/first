...
public class BusMsgAssNewsService implements IBusMsgAssNewsService {
	 ...
	 */
	@Override
	public String getGustyMessage() throws BusinessException {
	 ...
	 */
	@Override
	public String saveGustyMessage(String str) throws BusinessException {
		//转json格式
	 ...
	 */
	@Override
	public String saveHits(String str) throws BusinessException {
		JSONObject json = JSONObject.parseObject(str);
	...
	@Override
	public String saveSite(String str) throws ParseException {
		JSONObject json = JSONObject.parseObject(str);
	 ...
	 */
	@Override
	public String deleteReply(String str) throws BusinessException {
		JSONObject json = JSONObject.parseObject(str);
	...
	@Override
	public String saveReply(String str) throws BusinessException {
		String returnResult = "";
		JSONObject json = JSONObject.parseObject(str);
		logger.info("页面传过来的参数为:"+json);
		int isState = json.getInteger("isState");
		...
		if(isState<0){			
			...
			returnResult =  "{\"resultFlag\":false,\"failureReason\":\"传入isState的参数为NULL！\"}";
		}
		
		if(isState==0){
			returnResult = forIsState0(json, returnResult);
		}
		if(isState==1){
			returnResult = forIsState1(json,returnResult);
		}else{
			returnResult =  "{\"resultFlag\":false,\"failureReason\":\"参数isState不准确！\"}";
		}
		return returnResult;
		
	}
	private String forIsState1(JSONObject json,String returnResult) {
		logger.info("建议回复");
		...
		if(openId<0 || userNo==null || "".equals(userNo) || replyContent==null || "".equals(replyContent)){
			logger.info("{\"resultFlag\":false,\"failureReason\":\"传入的参数为NULL！\"}");
			returnResult =  "{\"resultFlag\":false,\"failureReason\":\"传入的参数为NULL！\"}";
		}
		...
		if(resId < 0){
			...
			returnResult = "{\"resultFlag\":false,\"failureReason\":\"保存信息失败！\"}";
		}
		returnResult =  "{\"resultFlag\":true,\"failureReason\":\"保存信息成功！\",\"id\":"+resId+"}";
		return returnResult;
	}
	private String  forIsState0(JSONObject json,String returnResult) {
		logger.info("评价回复保存");
		...
		if(retId<0 || userNo==null || "".equals(userNo) || replyContent==null || "".equals(replyContent)){
			logger.info("{\"resultFlag\":false,\"failureReason\":\"传入的参数为NULL！\"}");
			returnResult =  "{\"resultFlag\":false,\"failureReason\":\"传入的参数不正确！\"}";
		}
	...
	private int saveEvaluate(int retId,
			String userNo, String replyContent) throws BusinessException{
		//new 一个新的对象
