...
public class ProcScoringService implements IProcScoringService {
	...
	public String savaScoring(String str) throws BusinessException {
		...
		synchronized (this) {
			...
			if (StringUtil.isNotEmpty(useraddressid)) {
				//抽取方法
				result = setEngScoringEntity(scopeid, scope, date, engAddressEntity,
						engScoringEntity, useraddressid,result);
	...
	private String setEngScoringEntity(String scopeid, int scope, Date date,
			ProcAddressEntity engAddressEntity,
			ProcScoringEntity engScoringEntity, String useraddressid,String result) {
		// 有人员地点表ID修改时间
		engAddressEntity.setCreateDate(date);
		engAddressEntity.setId(useraddressid);
		// 修改
	...
	public String savaMessageReason(String str) throws BusinessException {
		...
		if (scope == 0) {
			if (//抽取方法
					checkIsBoolean(userNo, proAdress, proType, scopeid, proName,
					scopeName, scope, photo, damageReason, deductSProject,
					serviceItems)) {
				logger.info(RETURNVALUENULL);
				return RETURNVALUENULL;
			}
	...
	private boolean checkIsBoolean(String userNo, String proAdress,
			String proType, String scopeid, String proName, String scopeName,
			int scope, String photo, String damageReason,
			String deductSProject, String serviceItems) {
		return StringUtil.isEmpty(userNo) || StringUtil.isEmpty(proAdress)
				|| StringUtil.isEmpty(proType)
				|| StringUtil.isEmpty(scopeid)
				|| StringUtil.isEmpty(proName)
				|| StringUtil.isEmpty(scopeName) || scope < -1
	...
	private String getSavaScopetwo(
			...
			procMessageEntity,Date date) throws BusinessException{
