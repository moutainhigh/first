...
public class QueryPcInfoAction extends BaseAction {
		...
		try {
			...
			if(json==null){
				...
				if (StringUtils.isEmpty(pageSize)) {
					...
					//抽取公共方法
					setServeOriginatorsInfoEntity(newList, sdf, result,
							startProvinceCode, keyWord, carType, payType, list);
				}
			}
	...
	private void setServeOriginatorsInfoEntity(
			List<ServeOriginatorsInfoEntity> newList, SimpleDateFormat sdf,
			Result<List<ServeOriginatorsInfoEntity>> result,
			String startProvinceCode, String keyWord, int carType, int payType,
			List<ServeOriginatorsInfoEntity> list) throws BusinessException {
		...
		try {
			...
			if(null != json){
				...
				if (null != typePc && !"".equals(typePc)) {
					...
					//抽取公共方法
					setServeOriginatorsInfoEntityValue(newList, sdf, result, list); 
				}
			}
		} catch (Exception e) {
			//异常处理
	...
	private void setServeOriginatorsInfoEntityValue(
			List<ServeOriginatorsInfoEntity> newList, SimpleDateFormat sdf,
			Result<List<ServeOriginatorsInfoEntity>> result,
			List<ServeOriginatorsInfoEntity> list) throws BusinessException {
		if (null!=list &&list.size() > 0) {
