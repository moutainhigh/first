...
public class EmailAction extends BaseAction {
	...
	/**
	 * 草稿箱详情 使用
	 * 0 收件箱 1 已发送 2 草稿箱 3 废纸篓 4 发件箱 5 垃圾邮件
	 */
	private String type;
	...
	public void getDrafts() {
		...
		try {
			logger.info("工号" + userId + "查询草稿箱邮件");
			result = emailService.queryDrafts(page, pageSize, userId);
		} catch (Exception e) {
	...
	public void getSentEmail() {
		...
		try {
			logger.info("工号" + userId + "查询已发送邮件");
			result = emailService.querySentEmail(page, pageSize, userId);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void getOutbox() {
		...
		try {
			logger.info("工号" + userId + "查询发件箱邮件");
			result = emailService.queryOutbox(page, pageSize, userId);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	 ...
	 * 查询垃圾邮件
	 */
	public void getJunkEmail() {
		Result<Object> result = null;

		try {
			logger.info("工号" + userId + "查询垃圾邮件");
			result = emailService.queryJunkEmail(page, pageSize, userId);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void getDeletedItems() {
		...
		try {
			logger.info("工号" + userId + "查询已删除邮件");
			result = emailService.queryDeletedItems(page, pageSize, userId);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void getFolderCount() {
		...
		try {
			logger.info("工号" + userId + "查询各文件夹邮件数量");
			result = emailService.queryFolderCount(userId);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void searchEmail(){
		...
		logger.info("工号" + userId + "条件查询" + " condition=" + condition);
		Result<Object> result = emailService.searchEmailByCondition(userId,searchType,condition,more,emailType);
		//写入数据
		writeToPage(result);
	}
	...
	public void loadEmailById(){
		logger.info("工号" + userId + "根据邮件id加载邮件");
		Result<Object> result = null;
		if("".equals(type) || type == null){
			//原始的公共接口 不变
			result = emailService.loadEmailById(userId,emailId);
		}else{
			//草稿箱 邮件详情接口  使用type=2 时查看
			result = emailService.loadDraftsEmailById(userId,emailId,type);
		}
	...
	/**
	 * 将邮件从收件箱移动到已删除邮件  （收件箱+草稿箱+已发送+发件箱+垃圾邮件）
	 */
    @WhetherMonitorCutffPoint
	public void delete() {
    	logger.info("工号" + userId + "删除邮件");
		//声明一个数据
	...
	public void deleteDrafts() {
		...
		try {
			//删除数据
			result = emailService.deleteDrafts(userId, deleteIds);
			result.setData("删除已删除邮件成功");
		} catch (Exception e) {
			result.setErrorCode(ERRORCODE_3);
			result.setData("删除已删除邮件失败");
		}
		//写入数据
		writeToPage(result);
	}
	...
	public void downloadAttachment(){
		...
		try {
			...
			if("20160519".equals(version) && result.getErrorCode() == 0){
				logger.info("老的邮箱解密附件开始 》》》工号是"+userId);
				// 附件真实路径
...
//	}
   ...
   @WhetherMonitorCutffPoint
	public void newEmails() {
	    logger.info("工号" + userId + "发邮件newEmails");
		Result<Object> result = new Result<Object>();
	...
	public void saveDrafts() {
		...
		try {
			...
			if (null != mailSenderInfo
				...
				mailSenderInfo.setPassword(password);
				// 多个附件
				result = emailService.saveDrafts(mailSenderInfo, userId, files);
				result.setErrorMessage(Constants.ACTIVE_YES);
			} else {
				logger.info("保存草稿出错");
				result.setCount(Constants.WRONG_REQUEST);
			...
			e.printStackTrace();
			logger.info("保存草稿出错信息:" + e.getMessage());
			result.setCount(Constants.WRONG_REQUEST);
			result.setErrorMessage(Constants.ACTIVE_NO);
		}
		writeToPage(result);
   ...
   /**
	...
	public void sendDrafts() {
		...
		try {
			...
			if (null != mailSenderInfo
				...
				mailSenderInfo.setPassword(password);
				// 多个附件
				result = emailService.sendDrafts(mailSenderInfo, userId, files);
				result.setErrorMessage(Constants.ACTIVE_YES);
			} else {
				logger.info("邮件发送出错");
				result.setCount(Constants.WRONG_REQUEST);
	 ...
	 */
    @WhetherMonitorCutffPoint
	public void moveToJunkEmail() {
		//返回结果
		Result<Object> result = new Result<Object>();
		try {
			logger.info("工号" + userId + "设为垃圾邮件");
			result = emailService.moveToJunkEmail(userId,emailId);
		} catch (Exception e) {
			result.setErrorCode(ERRORCODE_3);
			result.setData("设为垃圾邮件失败");
			//日志
			logger.error("设为垃圾邮件出现异常>>>>>>>" +
	...
	public void newEmail() {
		logger.info("工号" + userId + "发邮件newEmail");
		Result<Object> result = new Result<Object>();
		MailSenderInfo mailSenderInfo = null;
		HttpServletRequest request = ServletActionContext.getRequest();
		String parameter = request.getParameter("sendInfo");
