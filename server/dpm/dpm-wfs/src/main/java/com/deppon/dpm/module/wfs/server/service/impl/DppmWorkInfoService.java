...
public class DppmWorkInfoService implements IDppmWorkInfoService {
	...
	public void updateProjectClosing(ApprovelEntity approvelEntity,IDpBpmsClientFacade clientFacade,
			ClosingIsPassEntity closingIsPassEntity)  {
...
//		String str = "\"participant\":\"202157|200635|091221\",\"activityDefid\":\"\",\"activityInstID\":0,\"workItemID\":0,\"processInstId\":0,\"isOver\":0";
	...
	private void doAfterApproval(IDpBpmsClientFacade client, 
			ApprovelEntity approvelEntity, ChangeCheckEntity changeCheckEntity)  {
...
//			if(changeCheckEntity != null) {
				...
				if(isLastApprove) {
					...
					changeEntity.setAuditState(Constants.CONSTANT_THREE);
					//审批同意后把变更信息同步到正式数据
					...
					if(changeCheckEntity.getChangeType() == 2) {
						if(changeCheckEntity.getProjectStatus() == Constants.CONSTANT_FOUR) { //项目状态变为终止
							//项目状态变为已终止，对应的任务也要终止
					...
					changeEntity.setAuditState(Constants.CONSTANT_FIVE);
				}
//			} else {
//				changeEntity.setAuditState(5);
//			}
		} else {
			changeEntity.setAuditState(Constants.CONSTANT_FOUR);
		}
		//修改审核状态
	...
	private void updateAnnualPlanFromTempToFormal(Integer oldAiId, Integer newAiId,
			Integer ghType, Integer changeState)   {
		if(changeState == Constants.CONSTANT_ONE) { //新增-执行插入操作（复制表数据）
			this.logger.info("---------------年度规划新增-dotp_gh_project_change--" + newAiId);
			try {
				this.changeDao.insertYearPlanProj(newAiId);
			} catch (Exception e) {
				logger.info("年度规划新增发生的异常",e);
				e.printStackTrace();
			}
		} else if(changeState == Constants.CONSTANT_TWO) {//修改
			//年度规划项目实体类
		...
		}else if(changeState == Constants.CONSTANT_THREE) {//删除原数据
