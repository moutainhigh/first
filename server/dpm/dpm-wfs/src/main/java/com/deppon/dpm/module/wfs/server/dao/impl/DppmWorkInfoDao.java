...
public class DppmWorkInfoDao extends iBatis3DaoImpl implements IDppmWorkInfoDao {
    ...
    public void insertClosingIndex(List<ClosingIndexEntity> list)  {
		...
		for(ClosingIndexEntity e : list) {
			sb.append("('").append(e.getBusino()).append("','").append(e.getObjective()).append("',")
			.append("'").append(e.getIndicator()).append("','").append(e.getFormula()).append("',")
			.append("'").append(e.getDeptProvider()).append("','").append(e.getDeptAudit()).append("',")
			.append("'").append(e.getObjState()).append("','").append(e.getIsShortTarget()).append("',")
			.append("'").append(e.getQuantitative()).append("','").append(e.getCompleteValue()).append("',")
			.append("'").append(e.getCompleteCondition()).append("','").append(e.getObjId()).append(")");
		...
		//执行更新
        this.dppmTemp.update(sb.toString());
    }
    /**
     * 审批通过时，插入是否结项通过
     * @param e
    ...
    public void insertClosingCheckType(List<ClosingCheckTypeEntity> list)  {
    	...
    	for(ClosingCheckTypeEntity e : list) {
    		sb.append("('").append(e.getBusino()).append("','").append(e.getCheckModule()).append("',")
    		.append("'").append(e.getCheckType()).append("','").append(e.getManagerName()).append("',")
    		.append("'").append(e.getManagerCode()).append("')");
    ...
    */
	public void approveProjInfo(ProjectEntity entity,String empCode) {
		...
		sql = "INSERT INTO dotp_pm_project_check_records(project_code,check_handler,check_suggestion,check_result,wfs_id," +
				...
				+ "','"+ap.getWorkItemId()+"'," + entity.getWfsIsOver() +"," + checkStatus + ")";
		//执行sql
		this.dppmTemp.update(sql);
		//存在下一个审批人
		...
		if(nextEmpCode != null && !nextEmpCode.equals("")) {
			...
			if(nextEmpCode.indexOf("|") > 0) {
				...
				for (int i = 0 ; i < nes.length ; i++ ) {
					sb.append("('").append(ap.getProcessinstid()).append("','").
					append(ap.getActivityDefId()).append("','").append(nes[i]).append("')");
				...
				sb.append("('").append(ap.getProcessinstid()).append("','").append(ap.getActivityDefId())
				.append("','").append(nextEmpCode).append("')");
		...
		int eaStatus = Constants.CONSTANT_FIVE; // 已同意3   不同意 4  审批中5
		if(entity.getWfsIsOver() == 1 ) {
			//如果审批结束 更新为 审批状态 同意或不同意
			if(ap.getIsAgree() != null && ap.getIsAgree().equals("0")) {
				//同意
				eaStatus = Constants.CONSTANT_THREE;
			} else {
				eaStatus = Constants.CONSTANT_FOUR;
			}
		}
		this.logger.info("当前工作流审批状态：(已同意3   不同意 4  审批中5)" + entity.getProjectCode() + "," + eaStatus);
		sql = "update dotp_pm_projects set ea_status =" + eaStatus + " where project_code = '" + entity.getProjectCode() + "'";
		//执行sql
    	this.dppmTemp.update(sql);
	}
    ...
    public Map<String, Object> orgallinfo(String sql) {
    	...
    	if(list != null && list.size() > 0 ){
    		for(ProjectOrgEntity o : list) {
    			...
    			if(o.getInternal() == Constants.PROJECT_LEAD) {
    				fqrinfo = o;
    			} else if(o.getInternal() == Constants.APROJECT_MANAGER) {
    				jfjlinfo = o;
    			} else if(o.getInternal() == Constants.BPROJECT_MANAGER) {
    				yfjlinfo = o;
    			} else if(o.getInternal() == Constants.INTERNAL_STAFF) {
    				//执行个人
			...
			public ProjectEntity mapRow(ResultSet rs, int rowNum)
    			...
    			if(overtaken != null && !overtaken.equals("")) {
    				...
    				if(overtaken.indexOf(",")>0) {
    					...
    					sb.append("'").append(overtaken).append("'");
    					//sql += "'" + overtaken + "'";
    				}
    				sb.append(")");
    				//sql += ")";
    				List<String> plist = dppmTemp.queryForList(sb.toString(),String.class);
	...
	public Map<String, Object> queryProjectResources(String sql) {
		...
		List<ProjectResources> projectResorcesList = this.dppmTemp.query(sql, new RowMapper<ProjectResources>(){
			...
			public ProjectResources mapRow(ResultSet rs, int rowNum)
				...
				projectResorcess.setManDay(rs.getInt("manDay")/Constants.TEN);
