...
public class WorkItemsListService implements IWorkItemsListService {
	...
	public Map<String, Object> workitemslist(WorkItemsDto dto) {
        ...
        java.util.regex.Matcher match=pattern.matcher(processdefname);
        if(match.matches()){
	...
	private Map<String, Object> getWorkFlowItem(
		...
		String yesterday = String.format("%tF",
				System.currentTimeMillis() - Constants.HOUR_MIN * Constants.THOUSAND * Constants.DAY_HOUR);
	...
	public Map<String, Object> getWorkFlowDateList(
		...
		for (Object obj : workflowdatelist) {
			if(obj.getClass() == WorkflowListEntity.class){
				if (((WorkflowListEntity)obj).getRequestInfo().getCreateTime().substring(0, Constants.TEN).equals(today)) {
					todayList.add(obj);
				} else if (((WorkflowListEntity)obj).getRequestInfo().getCreateTime().substring(0, Constants.TEN).equals(yesterday)) {
