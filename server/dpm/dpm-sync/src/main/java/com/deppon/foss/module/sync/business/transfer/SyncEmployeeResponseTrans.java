...
import com.deppon.foss.module.sync.business.jms.SendEmployeeResponse;
 ...
 * 异步员工响应转换
 */
public class SyncEmployeeResponseTrans implements
		IMessageTransform<SendEmployeeResponse> {
	/**
	 * The Constant CLZZ.
	 */
	...
	// LogFactory.getLog(SyncEmployeeResponseTrans.class);
	/**
	 * The context.
	 */
	...
	@Override
	// 空判断
	public SendEmployeeResponse toMessage(String string) throws ConvertException, UnsupportedEncodingException {
		if (context == null) {
			// 再次尝试一次
	 ...
	 */
	@Override
	public String fromMessage(SendEmployeeResponse value) throws ConvertException {
		// context为null
		if (context == null) {
			// 再次尝试一次
			JAXBContextUtil.initContext(CLZZ);
		...
		try {
			...
			// 序列化类创建
			JAXBElement<SendEmployeeResponse> element = new ObjectFactory().createSendEmployeeResponse(value);
			// 序列化
