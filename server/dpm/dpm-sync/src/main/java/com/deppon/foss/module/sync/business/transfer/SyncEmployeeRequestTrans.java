...
import com.deppon.foss.module.sync.business.jms.SyncEmployeeRequest;
 ...
 * 异步员工请求转换
 */
public class SyncEmployeeRequestTrans implements
		IMessageTransform<SyncEmployeeRequest> {
	/**
	 * The Constant CLZZ.
	 */
	private static final Class<SyncEmployeeRequest> CLZZ = SyncEmployeeRequest.class;
	/**
	 * The log.
	 */
	// private static Log log =
	// LogFactory.getLog(SyncEmployeeRequestTrans.class);
	/**
	 * The context.
	 */
	 ...
	 */
	@Override
	public SyncEmployeeRequest toMessage(String string) throws ConvertException, UnsupportedEncodingException {
		// 空判断
		if (context == null) {
			// 获取JAXBContext
			// 再次尝试一次
		...
		try {
			...
			// 反序列化
			JAXBElement<SyncEmployeeRequest> element = unmarshaller.unmarshal(new StreamSource(stream), CLZZ);
			// 获取对应的值
	 ...
	 */
	@Override
	public String fromMessage(SyncEmployeeRequest value) throws ConvertException {
		// context为null
		if (context == null) {
			// 再次尝试一次
			JAXBContextUtil.initContext(CLZZ);
		...
		try {
			...
			// 序列化类创建
			JAXBElement<SyncEmployeeRequest> element = new ObjectFactory().createSyncEmployeeRequest(value);
			// 序列化
