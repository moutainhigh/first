 ...
 */
public class WfsCheckUrlInterceptor extends MethodFilterInterceptor {
	...
	protected String doIntercept(ActionInvocation invocation) throws Exception {
		...
		Method method = ReflectionUtils.findMethod(target.getClass(),
				methodName);
		String wfsintercept = loginRedisService.get("wfsintercept");
		// 查看是否包含这个注解
		if (method.isAnnotationPresent(CookieNotCheckedRequired.class) || (wfsintercept != null && wfsintercept.equals("off"))) {
			// 包含这个注解直接返回，不走拦截器
