...
public class CheckUrlInterceptor extends MethodFilterInterceptor {
	...
	protected String doIntercept(ActionInvocation invocation) throws Exception {
		...
		if (StringUtils.isNotBlank(checkValue)) {
			...
			logger.info("工号" + userId + "获取到的checkValue为空");
			checkValue = loginRedisService
					.get(RedisService.DPM_LOGIN_LOGININFO_KEY + userId);
		}
		if (null != loginResult) {
		...
		if (value != null
				&& value.equals(bean)
				&& !"false".equalsIgnoreCase(loginRedisService.get("isValidSms"
						+ userId))) {
			try {
			...
			// 获取方法名
			String methodName2 = invocation.getProxy().getMethod();
			// 获取方法类
			Method method2 = ReflectionUtils.findMethod(target.getClass(),
					methodName);
			logger.info("会话过期的人工号" + userId);
			logger.info("会话过期的方法是" + methodName2);
			logger.info("会话过期的方法类是" + method2.toString());
			logger.info("传过来casSessionId是" + casSessionId);
			logger.info("传过来cookie是" + cookie);
			if (null != loginResult) {
				logger.info("获取到的casSessionId是" + loginResult.getSessionId());
				logger.info("获取到cookie是" + loginResult.getCasCookie());
			} else {
				logger.info("获取到loginResult");
			}
			...
			if ("/dpm/dpm/login_checkEmpStatus.action".equals(url)) {
				...
				EmpExtensionEntity empExtensionEntity = loginInfoService
						.checkStatus(userId, deviceToken);
				// 数据存储
	...
	public static void main(String[] args) {
		System.out.println("false".equalsIgnoreCase(null));
	}
}
