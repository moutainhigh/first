 ...
 * 
 * */
public class AutoPunchClockMonitorGlobalPcAction extends BaseAction implements ModelDriven<AutoPunchClockMonitorEntity>{
	...
	private static final Logger LOG = LoggerFactory.getLogger(AutoPunchClockMonitorGlobalPcAction.class);
	
	// 构建实体对象
	private AutoPunchClockMonitorEntity entity = new AutoPunchClockMonitorEntity();
	
	// service
	private IAutoPunchClockMonitorGlobalPcService autoPunchClockMonitorService;
	...
	public void queryByPage() {
		...
		try {
			...
			// 查询
			List<AutoPunchClockMonitorEntity> list = autoPunchClockMonitorService.queryByPage(start,rows,empCode);
			// 总条数
	...
	public void setAutoPunchClockMonitorService(
			IAutoPunchClockMonitorGlobalPcService autoPunchClockMonitorService) {
