 ...
 * 2018-09-07
 */
public class AppPermissionPcAction extends BaseAction{
	 ...
	 * 日志
	 */
	private final static Logger LOG = LoggerFactory.getLogger(AppPermissionPcAction.class);
	...
	//service
	private IAppPermissionPcService appPermissionPcService;
	...
	public void insertOrUpdate(){
		...
		if("save".equals(operation)){
			...
			try {
				...
				appPermission.put("status", status);
				//更新
				i = appPermissionPcService.update(appPermission);
				if( i == 1){
					//更新成功的状态码
	...
	public void delete(){
		...
		try {
			appPermissionPcService.deleteByApplyCodes(ids);
			//删除成功返回204
	...
	public void list(){
		...
		try {
			//查询列表数据
			list = appPermissionPcService.queryList(begin,rows);
		} catch (Exception e) {
			//日志
			LOG.error("查询app permission信息列表出错!!!", e);
		}
		//查询总的数据条数
		long count = appPermissionPcService.queryCount();
		//需要转为json的数据
	...
	//setter
	public void setAppPermissionPcService(
			IAppPermissionPcService appPermissionPcService) {
		this.appPermissionPcService = appPermissionPcService;
	}
