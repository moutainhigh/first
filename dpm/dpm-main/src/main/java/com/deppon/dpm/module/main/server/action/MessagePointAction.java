...
public class MessagePointAction  extends BaseAction{
	
	/**
	 * 构造方法
	 */
	public MessagePointAction(){
		super();
	}
	private static final Logger logger = LoggerFactory.getLogger(MessagePointAction.class);
	
	private static final long serialVersionUID = 1L;
	
	private static final int PAGESIZE_10 = 10;
	
	private IMessagePointService messagePointService;
	...
	public void getPoint(){
		...
		try {
			MessagePointVo pointVo = new MessagePointVo();
			pointVo = messagePointService.getPoint(appId);
			//new Date();
			/*int mark = 0;
			if(pointVo.getId() == 1){
			...
			*/
			...
			if(pointVo != null){
				SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd");
		    	//SimpleDateFormat sdf2 = new SimpleDateFormat("HH:mm:ss");
		    	Date day = sdf1.parse(sdf1.format(System.currentTimeMillis()));
		    	//String time = sdf2.format(System.currentTimeMillis());
		    	Date startdate = pointVo.getStartdate();
		    	Date enddate = pointVo.getEnddate();
		    	System.out.println(day.compareTo(startdate));
		    	System.out.println(day.compareTo(enddate));
	...
	public void getAllPoint(){
		...
		JSONObject jonum = new JSONObject();
		try {
			List<MessagePointVo> polist = new ArrayList<MessagePointVo>();
			polist = messagePointService.getAllPoint(getBegin(), pageSize);
			// 意见反馈的条数
			count = messagePointService.getCount();
			// 总共多少页
	...
	public void updatePointState(){
		...
		JSONObject jonum = new JSONObject();
		try {
			int i = messagePointService.updateState(pointId);
			jonum.put("return", i);
		} catch (Exception e) {
	...
	public IMessagePointService getMessagePointService() {
		return messagePointService;
	}

	public void setMessagePointService(IMessagePointService messagePointService) {
		this.messagePointService = messagePointService;
	}
