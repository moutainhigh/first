...
public class TravelAdvanceApplyService implements ITravelAdvanceApplyService {
	...
	public String queryTravelAdvanceApply() {
		...
		try {
			//设置参数开始时间
			long setRequestParamStartTime = System.nanoTime();
			Gson gson = new Gson();
			...
			//设置参数结束时间
			long setRequestParamEndTime = System.nanoTime();
			logger.info("queryTravelAdvanceApply---------设置调用请求携程接口参数，耗时：" + (setRequestParamEndTime - setRequestParamStartTime));
			long ctripRequestStartTime = System.nanoTime();
			//调用请求获取响应
			hclient.executeMethod(postMethod);

			rs = postMethod.getResponseBodyAsString();
			long ctripRequestEndTime = System.nanoTime();
			logger.info("queryTravelAdvanceApply---------调用携程接口获取ticket，耗时：" + (ctripRequestEndTime - ctripRequestStartTime));
			JSONObject jsonObj = JSONObject.parseObject(rs);
			rs = jsonObj.toJSONString();
			logger.info("queryTravelAdvanceApply---------调用携程接口获取ticket,返回值为：" + rs);
		} catch (Exception e) {
